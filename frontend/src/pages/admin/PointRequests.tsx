...
export default function PointRequests() {
  ...
  // 통계 계산
  const chargePending = pointChargeRequests.filter(req => req.status === 'pending').length;
  const chargeApproved = pointChargeRequests.filter(req => req.status === 'approved').length;
  const chargeRejected = pointChargeRequests.filter(req => req.status === 'rejected').length;
  
  const withdrawPending = pointWithdrawRequests.filter(req => req.status === 'pending').length;
  const withdrawApproved = pointWithdrawRequests.filter(req => req.status === 'approved').length;
  const withdrawRejected = pointWithdrawRequests.filter(req => req.status === 'rejected').length;

  const totalPending = chargePending + withdrawPending;
  const totalProcessed = chargeApproved + chargeRejected + withdrawApproved + withdrawRejected;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-yellow-100">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-yellow-600 text-sm font-semibold uppercase tracking-wide">대기 중</p>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-green-100">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-green-600 text-sm font-semibold uppercase tracking-wide">처리 완료</p>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-blue-100">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-blue-600 text-sm font-semibold uppercase tracking-wide">충전 대기</p>
                  <p className="text-3xl font-bold text-gray-900 mt-2">
                    {(totalChargeAmount / 10000).toFixed(0)}만원
                  </p>
                  <p className="text-gray-500 text-sm mt-1">{chargePending}건 대기</p>
                </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-red-100">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-red-600 text-sm font-semibold uppercase tracking-wide">환전 대기</p>
                  <p className="text-3xl font-bold text-gray-900 mt-2">
                    {(totalWithdrawAmount / 10000).toFixed(0)}만원
                  </p>
                  <p className="text-gray-500 text-sm mt-1">{withdrawPending}건 대기</p>
                </div>
        ...
        <div className="mt-12 bg-white rounded-2xl p-8 shadow-lg border border-gray-100">
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              <h3 className="text-xl font-bold text-gray-900 mb-3">💡 관리 가이드</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-600">
                <div>
                  <h4 className="font-semibold text-gray-800 mb-2">충전 신청 처리</h4>
                ...
                <div>
                  <h4 className="font-semibold text-gray-800 mb-2">환전 신청 처리</h4>
                  <ul className="space-y-1 text-sm">
